...
import { createAudioMixerPlayer } from './audioMixerPlayer';
import type { Track } from '@/types/mixer';

type BufferSourceHarness = {
...
function createHarness() {
  let masterGainNode: GainHarness | null = null;
  const trackGainNodes: GainHarness[] = [];
  const sources: BufferSourceHarness[] = [];
  ...
  return {
    ...
    availableTracks,
    fetchImpl,
    masterGainNode: (): GainHarness | null => masterGainNode,
    sources,
    trackGainNodes
  };
}

function createAudioContextCtor(harness: ReturnType<typeof createHarness>) {
  return class AudioContextCtor {
    currentTime = harness.audioContext.currentTime;
    destination = harness.audioContext.destination;
    decodeAudioData = harness.audioContext.decodeAudioData;
    createBufferSource = harness.audioContext.createBufferSource;
    createGain = harness.audioContext.createGain;
  };
}
...
describe('audioMixerPlayer', () => {
  it('loads only the selected tracks when realtime mode is off', async () => {
    ...
    const player = createAudioMixerPlayer({
      AudioContextCtor: createAudioContextCtor(harness),
    ...
    expect(harness.masterGainNode()).not.toBeNull();
